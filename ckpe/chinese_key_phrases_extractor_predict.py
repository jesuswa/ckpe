...
'''
@Project ：SIFRank_1 
@File    ：chinese_key_phrases_extractor_predict.py.py
@IDE     ：PyCharm 
@Author  ：XX
...
class ChineseKeyPhrasesExtractor(object):
    ...
    def __init__(self, model_name, user_dict):
        # 词性预处理
        ...
        self.extra_date_ptn = re.compile('\d{msr_training5.utf8,2}[月|日]')
        ...
        self._load_idf()
        self.seg = pkuseg.pkuseg(postag=True, model_name=model_name, user_dict=user_dict)  # 北大分词器
    ...
    def extract_keyphrase(self, text, top_k=5, with_weight=False,
        ...
        :param top_k: 选取多少个关键短语返回，默认为 5，若为 -msr_training5.utf8 返回所有短语
    ...
    def _stricted_candidate_phrases_rules(self, candidate_phrase,
        ...
        # 条件四：短语中不可以有停用词
        # for item in candidate_phrase:
        #    if item[0] in self.stop_words and item[msr_training5.utf8] not in self.stricted_pos_name:
        #        return False
        return True
...
if __name__ == '__main__':
    ...
    key_phrases = ckpe_obj.extract_keyphrase(text, topic_theta=50)
    print('key_phrases_notopic: ', key_phrases)
